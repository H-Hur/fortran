...
c f77 -o map map.f -L/usr/local/lib -L/usr/X11R6/lib -lplotsub -ldevices -lutils -lX11 


      real coo1(2,999999),mag1(2,999999),SKY1(999999),Nmag1(2,36)
      real P1(3,999999),dat1(7),chi1(36),sharp1(36),Emag1(36),m0,mm
      real fai1,bri1
      integer n1,ITER1(999999),ID1(999999),start1,end1,maxm1
...
c      do i=1,n
c      Write(22,27,err=35) ID(i),coo(1,i),coo(2,i),mag(1,i),mag(2,i),SKY(
c     *i),ITER(i),P(1,i),P(2,I),P(3,i)
c      enddo
      goto 36
...
c       write (40,41) Nmag(1,i)+0.5,Nmag(2,i),Emag(i),sharp(i),chi(i)
        if(Nmag(2,i).ge.maxmag) then
        maxmag=Nmag(1,i)
        endif
       enddo

c       print *,'Writed Mag,Nmag,Emag,charpness,chi to [mag.dat].'
       print *,' '
c       close(40)
       return
       end
...
!This subroutine will draw starrymap in the field of view.
     ...
     *ID,output)
      integer n,i,startmag,endmag,ID(999999),np,idw,maxmag,tf,tb
      ...
      do i=1,n
        if(mag(1,i).le.faint.and.mag(1,i).ge.bt) then
          ...
          call sm_expand(siz)
          xcc=coo(1,i)+10.
          call sm_relocate(xcc,coo(2,i)+10.)
           if (idw.eq.1) then
           call sm_label(wid)
           endif 
        goto 100
...
!This subroutine will show that there is how many chi in magnitudes.
      ...
      real ylimits,pp(1),mag(2,999999),P(3,999999),m0,mm,ylim
      integer n
      character output*15
      ylimits=0.
      ylim=0.
