...
c f77 -o 'ccd.exe' 'ccd.f' -L/usr/local/lib -L/usr/X11R6/lib -lplotsub -ldevices -lutils -lX11 
      ...
      character H*1,input*16,output*23,fil(ni)*10,ch(10),filter(ni)*1
...
100   write(xlabel,'(5a1)') filter1,' ','-',' ',filter2
     ...
     * mag(i,yaxis1).gt.0.d0.and.mag(i,yaxis2).gt.0.d0) then
      ...
      enddo
c      print *,lx,mx,ly,my
      write(output,101) 'postportfile ',filtery1,'-',filtery2,'.',
     *filter1,'-',filter2,'.ps'
