...
c      write(coeff,'(a16)') 'coeff.302.fig   '
      ...
      parameter (nf=30,ns=100000)
      integer ntrans,nreiter,niter,infor_iter(nf,ns),set_f
      character filter(nf)*10,cname(nf)*10,H*1
      real*8 airmass(nf),UT(nf)
      real*8 color(nf),Q,slope,red(ns,2),wt,wtsum
      real*8 ebv_mean,ebv_mead0,ncorred,bv1,ub1
      real*8 c_bv(21),c_u(21),fbv,dmag(nf),omag1(nf)
      real bv0,ub0
...
c     write head
      ...
      print *,'Do you want to correct the non-linear transform of Lando
     &lt U filter? (yes = 1)'
      read(*,*) set_f
...
300   niter=0
        ...
        write(41,'(30f10.3)') (ebv_mean,j=1,nfilter)
        write(41,*) ""
        write(41,*) ""
       enddo
      close(41)
...
300   close(31)
      ...
      parameter (nf=30,ns=100000,maxiter=1000)
      common /zams/zMv(ns),zVI(ns),zBV(ns),zUB(ns),nzams
      real*8 dy,unredx,sBV,sUB,slope,eBV,eUB,Rv
      real sbv1,sub1,fx,fy,unredy,dx
      slope=0.72
      Rv=3.1
      sbv1=sBV
      sub1=sUB
      call lint(sub1,zUB,zBV,fx,nzams,nzams)
      dx=fx-sBV
      call lint(sbv1,zBV,zUB,fy,nzams,nzams)
      dy=sUB-fy
      unredx=sBV
      unredy=sUB
      do j=1,1000
       call lint(unredy,zUB,zBV,fx,nzams,nzams)
       dx=fx-unredx
       dy=dx*slope
       unredx=unredx+dx
       unredy=unredy+dy
