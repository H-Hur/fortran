c     gfortran -o aex_lint ~/work/fortran/subroutine/aex/aex_lint/aex_lint.f                                     
...
c      common /zams/zMv(34),zVI(34),zBV(34),zUB(34)
     ...
     &,add3_dp(nf)*16
      ...
      real o_Icol,col_I,e_V
      integer nUB,nBV,nVR,nVI,nRI,nback,ncal_v,niter,nid,nit(ns)
...
121    if(niter.ge.20) nmax=nmax+1
     ...
     &  UB,BV,VR,VI,RI,airmass(nc_I),UT(nc_I),xc(i),yc(i),o_Icol)
        if(o_Icol.gt.17) then
         nid=nid+1
c         write(42,50)i,nid,xc(i),yc(i),o_V,o_Icol,UB,BV,VI,RI,o_Ha,
c     &   cmerr(nc_V,i),cmerr(nc_I,i),e_UB,e_BV,e_VI,e_RI,cmerr(nc_Ha,i),
c     &   nobs(nc_V,i),nobs(nc_I,i),nUB,nBV,nVI,nRI,nobs(nc_Ha,i)
         nit(nid)=niter
         mean_iter=mean_iter+niter
        endif
       endif
...
c       if(nfil.eq.1.or.nfil.eq.2) then!.or.nfil.eq.5) then
c        write(*,'(15f9.4)') omag,cmag1,aeK1(nt),aeK2(nt),
c     &  col,X,zero(nt),dZ_col,dZ_ut,dZ_rad,dZ_a1!,ub-0.72*bv
cc       endif
      return
      end
...
c      open(23,file='Sung2001.ZAMS.cc')
      read(23,'(a1)') H
      do i=1,41
c      do i=1,34
...
c100   call lint(ub1,zUB,zBV,bv0,34,34)
