...
c     06,30,2008 last updated by Hur Hyeon-Oh
      ...
      print *,'Input data file name=? ( 1 = wcout2.fs)'
      read(*,'(a16)') input
      if(input.eq.'1') then 
       write(input,'(a16)') 'wc302_tr16.fs   '
      endif
      call readwchead(input,airmass,UT,filter)

c      write(coeff,'(a16)') 'coeff.302.fig_md'
...
c     write head
      ...
      write(41,*) ' Mean e(B-V) from early type stars, nobs'
      write(41,*) ""
      write(41,*) ""
...
c            if(i.eq.11) then 
c             print *,ovvi,dvvi,cvvi,color(nc_V)
c            endif
           endif
           if(UT(nc_V).ge.timeran(k,1).and.UT(nc_V).lt.timeran(k,2).and.
     &     color(nc_V).ge.colorran(k,1).and.color(nc_V).lt.colorran(k,2)
...
c            if(i.eq.11) then
c             print *,oiri,diri,ciri,color(nc_I)
c            endif
           endif
     ...
     &      +zeropoint(k)
        ...
        nreiter=0
        do j=1,nfilter
         if(dmag(j).gt.0.000001d0) then
          nreiter=nreiter+1
         endif
         if(niri.ne.0.and.diri.gt.0.000001d0) then
          nreiter=nreiter+1
         endif
         if(nvvi.ne.0.and.dvvi.gt.0.000001d0) then
          nreiter=nreiter+1
         endif
        enddo
...
400   do i=1,nstars
       omag(1,i)=omag(1,i)-0.0104d0
       omag(2,i)=omag(2,i)-0.0097d0
       omag(3,i)=omag(3,i)+0.0320d0
       omag(4,i)=omag(4,i)+0.0225d0
       omag(5,i)=omag(5,i)+0.0307d0
      enddo
      ...
      do i=1,nstars 
        ...
        write(41,'(t1,f10.3,30i10)') ebv(i),(nobs(j,i),j=1,nfilter)
        write(41,*) ""
        write(41,*) ""
        write(41,*) ""
        write(41,*) ""
