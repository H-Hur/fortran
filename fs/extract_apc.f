c   gfortran -o extract_apc ~/work/fortran/subroutine/fs/extract_apc.f  
c   21,31,32 
      parameter(ns=100000)
      character input*32,output*32,ch(32)*1
      real*8 aprad,axc(ns),ayc(ns),amerr(ns),amag(ns)
      real*8 xc(ns),yc(ns),mag(ns),merr(ns)
      integer nstar,id(ns),nap
      print *,'Input apc file=?'
      read(*,'(a32)') input
      print *,'aperture=?' 
      read(*,*) aprad  
      call readapc(input,aprad,axc,ayc,amag,amerr,nap)
      call read_als(xc,yc,mag,merr,ID,nstar)
      print *,'output=?'
      read(*,*) output

      call print_apcor(output,axc,ayc,amag,amerr,nap,
     &xc,yc,mag,merr,ID,nstar)


      stop
      end
  
      subroutine print_apcor(output,axc,ayc,amag,amerr,nap,
     &xc,yc,mag,merr,ID,nstar)
      parameter(ns=100000)
      real*8 aprad,axc(ns),ayc(ns),amerr(ns),amag(ns)
      real*8 xc(ns),yc(ns),mag(ns),merr(ns),cor,cor_e,aerr,awt
      integer nstar,id(ns),nap
      character output*32
      open(32,file=output)
      write(32,'(a70)') '#   apc_xc    apc_yc    apcor  apcor_wt       
     &                                                                 '
      do i=1,nap
       do j=1,nstar
        if(axc(i).eq.xc(j).and.ayc(i).eq.yc(j)) then
          cor=amag(i)-mag(j)
          if(amerr(i).eq.0.) amerr(i)=0.001d0
          aerr=sqrt(amerr(i)**2+merr(j)**2)
          awt=0.0001d0/aerr/aerr
          if(awt.gt.1.d0) awt=1.d0
          write(32,'(2f10.3,2f9.3)') axc(i),ayc(i),cor,awt
         goto 100
        endif 
       enddo
100   i100=i100
      ...
      subroutine readapc(input,aprad,axc,ayc,amag,amerr,nap)
      parameter (ns=100000)
      ...
      real*8 ayc(ns),amerr(ns),amag(ns),pmag(ns)
      corap=aprad
      open(31,file=input)
      nhead=0
      do i=1,100
...
31    print *,'Failed to start reading from ',input
32    do i=1,ns
