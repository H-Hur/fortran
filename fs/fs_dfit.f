...
c  f95 -o fs_dfit /home/gjgusdh/fortran/subroutine/fs/fs_dfit.f
      ...
      integer ID(ni,ns),nstar(ni),sid(ni,3),nimage,nstars,inum(ns)
      character input(ni)*16,filter(ni)*16,infs*16,inapc(ni)*16,h(16)*1
      integer mpart(ns,ni,10),tpart(ns,ni,10),IDF(ns,ni),mas(ni)
      integer iid(ns)              
...
C Finding same stars on each images.
      ...
      do n=2,nimage
       call fs(n,input,filter,xc,yc,dx,dy,cmag,merr,mpart,tpart,ID,
     *sid,MAGF,IDF,sharp,chi,nstars,nstar,dfit,inum,iid)!change mag->cmag to use
c APC.
      enddo
      open(65,file='dm.fs')
      write(65,'(t1,a1)') ' '
...
c To find same stars on other images.
     ...
     *,ID,sid,MAGF,IDF,sharp,chi,nstars,nstar,dfit,inum,iid)
      parameter(ns=100000,ni=30)
      ...
      integer tpart(ns,ni,10),nfit,tarID,mpart(ns,ni,10),inum(ns)
      integer nstars,iid(ns)
      character input(ni)*16,filter(ni)*16,H*16,output*20,ch(20)*1
      real*8 xcen,ycen,disdance,dist_c,xc_c,yc_c,tx(ni,ns),ty(ni,ns)
...
90    format('fitrad=',f4.1,'...fitting...')
     ...
     * dy(ninput,3)*xc(ninput,i)
       tx(ninput,i)=refx
       ty(ninput,i)=refy
       dist_c=sqrt((xc_c-refx)**2.d0+(yc_c-refy)**2.d0)
       if(dist_c.le.500.d0) then
        fit=1.5d0
       elseif(dist_c.le.900.d0) then
        fit=2.0d0
       else
        fit=dfit(ninput)
       endif
     ...
     *    **0.5d0
         ...
         pu(i)=used(j)
         fitten(ninput,i)=
     &   sqrt((refx-xc(1,j))**2.d0+(refy-yc(1,j))**2.d0)
         if(npartner(i).eq.1) nfit=nfit+1
         if(npartner(i).eq.2) nfit=nfit-1
        endif  
       enddo
      enddo
       print *,'            '
...
105   format(I6, T9,F8.3, T19,F8.3, T29,I3, T37,F5.3,f10.3,T54,I1)
120   do i=1,nstars
...
121    MAGF(i,ninput,1)=xc(ninput,j)
     ...
     *  *xc(ninput,i)
      ...
      write(31,*) ''
      write(31,*) '   ID     xc        yc   npartner  dist_c  fitrad ref
     &npartner'

      do i=1,nstar(ninput)
       dist_c=sqrt((xc_c-xc(ninput,i))**2.d0+(yc_c-yc(ninput,i))**2.d0)
       write(31,105)ID(ninput,i),xc(ninput,i),yc(ninput,i),
     * npartner(i),fitten(ninput,i),dist_c,pu(i)
      enddo
      close(31)       
      return
      end 
        ...
        if(tpart(i,k,2).eq.0) then
         ...
         do j=1,10
          if(tpart(i,k,j).eq.0) then
           ...
           write(31,100) tpart(i,k,j),(tar(l),l=1,4),dist
         enddo
150      write(31,*) ''
        endif
       enddo
...
9     nh=i-1
      ...
      do i=1,nh
       write(41,'(t1,a80)') H(i)
      enddo
      write(41,'(t1,a80)') '#im ID(ap)   ID   xc       yc     psfmag apm
...
c performing aperture correction.
     ...
     *  (yc(nimage,i)-apyc(nimage,j))**2.d0)
        ...
        elseif(apcerr(nimage,j).gt.0.03d0) then
         we= 0.d0
c         print *,j,'st apc star of the image',nimage,' were rejected.'
        else
         we= 0.005d0 / apcerr(nimage,j)**2
        endif
        wd=exp(-dist/500.d0)
...
100   nfilt=i-1
     ...
     &   (MAGF(i,ims(j),5)+dm(ninput,j)-wcmag(ninput,i))**2*wt
     &     
        enddo
        if(npoint.ge.2) then
