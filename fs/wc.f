...
c  f77 -o wc /home/gjgusdh/fortran/subroutine/fs/wc.f -L/usr/local/lib -L/usr/X11R6/lib -lplotsub -ldevices -lutils -lX11 
      ...
      print *,'input output from fs file=  ? ( 1 = out2.fs )'
      read(*,'(a16)') inwc
      if(inwc.eq.'1') write(inwc,'(a16)') 'out2.fs               '
      call read_out_fs(inwc,nimage,input,filter,IDF,MAGF,mas,air,
     &psf,ut,nstars)
      print *,'input fs file =? ( 1 = input.fs )'
      read(*,'(a16)') fs
      if(fs.eq.'1') write(fs,'(a16)') 'input.fs              '
      open(21,file=fs)
      do i=1,nimage
       read(21,'(t70,i1)') mas(i)
      enddo
      close(21)
      do i=1,nimage
     ...
     &nfc(ni,2),IDW(ni,ns),ims(ni)
      ...
      do i=1,10       
       ...
       call wcomb(nf(i),mas,IDF,MAGF,air,nstars)
100    nc=nc
      enddo   
      return
      end 
...
c     &   (MAGF(j,ims(i),6)**2.d0+MAGF(j,master(nf),6)**2.d0))
         wt1=0.01d0/MAGF(j,ims(i),6)/MAGF(j,ims(i),6)
         wt2=0.01d0/MAGF(j,master(nf),6)/MAGF(j,master(nf),6)
...
c      if(dm(nf,i).ne.0.) then
       write(ps,'(t1,a13,1x,i1,a1,i1,a6)') 
     & 'postlandfile ',nf,'-',nfimage,'_dm.ps                          '
       call sm_device (ps)
       call sm_graphics
       call sm_erase()
       x1=mmin2
       x2=mmax2
       call sm_limits(x1,x2,-0.3,0.3)
       call sm_box(1,2,0,0)
       call sm_relocate(x1,dm(nf,nfimage))
       call sm_draw(x2,dm(nf,nfimage))
       call sm_xlabel('Mag')
       call sm_ylabel('\\gDMag(Master-target)')
       do i=1,nc
        if(ww(i).gt.0.99) ww(i)=0.99
        if(ww(i).lt.0.01) ww(i)=0.01
        if(nt(i).eq.1) then
         call sm_ptype(993.+ww(i),1)
        else
         call sm_ptype(993.+ww(i),1)       
        endif
        call sm_points(mag_mas(i),dm2(i),1)
c        print *,mag_mas(i),dm2(i)
       enddo
       write(nn,'(i5)') nit
       call sm_relocate(x1+2.,0.2)
       call sm_label(nn)
       call sm_gflush()
       call sm_hardcopy
       call sm_alpha
c      endif
      ...
      subroutine wcomb(ninput,mas,IDF,MAGF,air,nstars)
      parameter (ns=100000,ni=30)
...
100   nfilt=i-1
     ...
     &   wer_er=wer_er+1./(MAGF(i,ims(j),6)**2)
       ...
       if(npoint.ne.0.and.wcmerr(ninput,i).le.0.001) then
        write(*,'(i3,15f9.3)') npoint,
     &  (MAGF(i,ims(j),5)+dm(ninput,j),MAGF(i,ims(j),6),j=1,nfilt)
     &  ,wcmag(ninput,i),wesum,wsum,wcmerr(ninput,i)
       endif
      enddo
      return
      end
...
500   close(51)
     ...
     &psf,ut,nstars)
      ...
      open(21,file=inwc)
      read(21,'(t2,30a10)') (input(j),j=1,ni)
      do i=1,ni
       psf(i)=0.d0
      enddo
      do i=1,100
