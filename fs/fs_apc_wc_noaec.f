...
C reading als and apc files.
     ...
     * nstar) 
      ...
      do i=1,nstar(1)
       ...
       MAGF(i,1,3)=0.d0
       MAGF(i,1,4)=0.d0
       MAGF(i,1,5)=cmag(1,i) !!!!!!!!!!!chamge to mag->cmag
       MAGF(i,1,6)=merr(1,i)
...
C Finding fake candidates.
      call writefake(nimage,filter,ID,xc,yc,dx,dy,tpart,cmag,merr,
     *nstars,nstar,input)
...
c performing aperture correction.
     ...
     *  (yc(nimage,i)-apyc(nimage,j))**2.d0)
        if(apcerr(nimage,j).le.0.01d0) then
         we=1.d0
        elseif(apcerr(nimage,j).gt.0.01d0) then
         we=10.d0**(1.d0-apcerr(nimage,j)/0.01d0)
        endif
        wd=exp(-dist/150.d0)
...
c       print *,nimage,i,apsum,wsum,apsum/wsum,mag(nimage,i)
      enddo
      return
      end
