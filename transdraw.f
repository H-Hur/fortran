...
c f77 -o 'transdraw.exe' 'transdraw.f' -L/usr/local/lib -L/usr/X11R6/lib -lplotsub -ldevices -lutils -lX11
      parameter (nf=10,ns=1000)
      character filter(nf)*2,star(nf,ns)*16,input*16,coeff*16,output*32
      real*8 transy(nf,ns),airy(nf,ns),timey(nf,ns),colx(nf,ns)
      real*8 timex(nf,ns)
      integer num_f(nf),num_c(nf),ntransform,lastnum_f,ne,nne

      write(output,'(a32)') 'postportfile transdraw.ps       '
c      write(output,'(a32)') 'x11'
      ...
      do ndate=1,20
       ...
       if(i.le.ntransform-1) then
        ...
        if(num_f(i).eq.1) then!U
         print *,'Drawing U...'
         call sm_location(6000,30000,27000,30000)
         call transdraw(star,filter,transy,colx,num_f(i),num_c(i),i,ne) 
        elseif(num_f(i).eq.2) then!B
         print *,'Drawing B...'
         call sm_location(6000,30000,21000,24000)
         call transdraw(star,filter,transy,colx,num_f(i),num_c(i),i,ne)
        elseif(num_f(i).eq.3) then!V
         print *,'Drawing V...'
         call sm_location(6000,30000,15000,18000)
         call transdraw(star,filter,transy,colx,num_f(i),num_c(i),i,ne)
        elseif(num_f(i).eq.4) then!R
         print *,'Drawing R...'
         call sm_location(6000,30000,9000,12000)
         call transdraw(star,filter,transy,colx,num_f(i),num_c(i),i,ne)
        elseif(num_f(i).eq.5) then!I
         print *,'Drawing I...'
         call sm_location(6000,30000,3000,6000)
         call transdraw(star,filter,transy,colx,num_f(i),num_c(i),i,ne)
        endif
       enddo
...
900   print *,'Close the output file.'
      call sm_gflush()
      call sm_hardcopy
      call sm_alpha
     ...
     &transcoeff(numtr)
      ...
      do i=1,npoint
       if(x(i).gt.xmin.and.x(i).le.xmax) then
        ...
        if(w(i).ge.wei_mean*1.5d0) then
         write(label,'(a20)') star(mfilter(numfil),i)
         call sm_expand(0.4)
         call sm_relocate(x(i)-0.1*xran,y(i)+0.2*yran)
c         call sm_label(label)
        endif
       endif
      enddo
...
c            print *,airy(i,j)
     ...
     &    num_fil(k).eq.6) then
      ...
      subroutine readcoeff(coeff,num_f,num_c,ntrans)
      parameter (nf=10,ns=1000)
